...
def go(args):
    ...
    logger.info(
        f"Retrieved data has {df.shape[0]} records and {df.shape[1]} " "columns"
    )

    # Filter on price
    df = df.loc[df["price"].between(args.min_price, args.max_price)]
    logger.info(
        f"Filter out records with price not between {args.min_price} "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_type", type=str, help="Output artifact's type", required=True
    )
    ...
    parser.add_argument(
        "--min_price", type=float, help="Minimum price acceptable", required=True
    )

    parser.add_argument(
        "--max_price", type=float, help="Maximum price acceptable", required=True
    )
