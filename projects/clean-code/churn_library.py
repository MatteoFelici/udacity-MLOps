...
from constants import CAT_COLUMNS, QUANT_COLUMNS, ATTRITION_COLUMN, FIGSIZE, PARAM_GRID

# Logger
logging.basicConfig(
    format="%(asctime)s : %(levelname)s : %(message)s",
    level=logging.INFO,
    handlers=[logging.FileHandler("churn_log.log"), logging.StreamHandler()],
)
...
class ChurnModel:
    ...
    def perform_eda(self, figsize: tuple = FIGSIZE, kde: bool = False):
        """
        ...
        for feat in all_features:
            ...
            if feat == self.attrition_flag:
                ...
                sns.heatmap(
                    self.input_data[self.numerical_columns].corr(),
                    annot=False,
                    cmap="Dark2_r",
                    linewidths=2,
                )
                ...
                logging.info(
                    f"Barchart on categorical variable {feat} with "
                    "relative frequency"
                )
    ...
    def target_encoder_fit(self, input_data, label_data):
        ...
        for i, feature in enumerate(self.category_columns):
            j = i + 1
            logging.info(
                f"Encode categorical feature {feature} - {j} of " f"{len_category_lst}"
            )
            # Calculate encodings on input data
            self.encodings[feature] = (
                pd.concat((input_data[feature], label_data), 1)
    ...
    def perform_feature_engineering(self, test_size: float = 0.3):
        """
        ...
        logging.info(
            f"Encode {self.attrition_flag} feature into binary " "feature Churn"
        )
        self.input_data["Churn"] = 1
        self.input_data.loc[
            self.input_data[self.attrition_flag] == "Existing Customer", "Churn"
        ] = 0
        ...
        # Split train-test
        logging.info(f"Split input data into train/test ({test_size} test " "size)")
        x_train, x_test, y_train, y_test = train_test_split(
            self.input_data.drop("Churn", 1),
            self.input_data["Churn"],
            test_size=test_size,
            random_state=42,
    ...
    def train_models(self, param_grid: dict = PARAM_GRID):
        """
        ...
        cv_rfc = GridSearchCV(
            estimator=self.rfc, param_grid=param_grid, cv=5, verbose=3
        )
        ...
        y_train_preds_rf = self._predict_and_store(
            self.rfc, x_train, "data/rfc_preds_train.csv"
        )
        y_test_preds_rf = self._predict_and_store(
            self.rfc, x_test, "data/rfc_preds_test.csv"
        )

        logging.info("Calculate predictions for logistic regression")
        y_train_preds_lr = self._predict_and_store(
            self.lrc, x_train, "data/lrc_preds_train.csv"
        )
        y_test_preds_lr = self._predict_and_store(
            self.lrc, x_test, "data/lrc_preds_test.csv"
        )
        ...
        self.feature_importance_plot(
            self.rfc, x_test, "images/rfc_feature_importances.png"
        )
    ...
    def feature_importance_plot(
        estimator, input_data: pd.DataFrame, output_pth: str, figsize: tuple = FIGSIZE
    ):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--input-path", help="Path to input data", default="./data/bank_data.csv"
    )
