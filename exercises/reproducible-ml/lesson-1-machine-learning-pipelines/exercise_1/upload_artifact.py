...
def go(args):
    ...
    # Create an instance of the class ``wandb.Artifact``. Use the
    # ``artifact_name`` parameter to fill the keyword ``name`` when constructing
    # the wandb.Artifact class.
    # Use the parameters ``artifact_type`` and ``artifact_desc`` to fill
    # respectively the keyword ``type`` and ``description``
    ...
    # instance using ``artifact.add_file``, and log the artifact to the run
    # using ``run.log_artifact``.
